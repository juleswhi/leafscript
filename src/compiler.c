//
// Created by ayo mr white on 19/10/2023.
//
...
ParseRule rules[] = {
        ...
        [TOKEN_GREATER]       = {NULL,     binary,   PREC_COMPARASIN},
        [TOKEN_GREATER_EQUAL] = {NULL,     binary,   PREC_COMPARASIN},
        [TOKEN_LESS]          = {NULL,     binary,   PREC_COMPARASIN},
        [TOKEN_LESS_EQUAL]    = {NULL,     binary,   PREC_COMPARASIN},
...
static void block() {
    ...
    consume(TOKEN_RIGHT_BRACE, "Expect a '}' after a block");
}
...
static void forStatement() {
    beginScope();
    consume(TOKEN_LEFT_PAREN, "Expected a '(' after a 'for'");
...
static void ifStatement() {
    consume(TOKEN_LEFT_PAREN, "Expected a '(' after an 'if'");
    expression();
...
static void whileStatement() {
    int loopStart = currentChunk()->count;
    consume(TOKEN_LEFT_PAREN, "Expected a '(' after a 'while'");
    expression();
    consume(TOKEN_RIGHT_PAREN, "Expected a ')' after a condition");
